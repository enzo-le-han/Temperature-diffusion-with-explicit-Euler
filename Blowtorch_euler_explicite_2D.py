...
sigmax = 5
sigmay = 5
x0 = int(np.around(Lx / 4))
y0 = int(np.around(7*Ly / 8))
...
def square_window_2d(x, y, xm, xM, ym, yM):
    M = np.zeros((Ny, Nx), dtype=float)
    for i in range(Nx):
        for j in range(Ny):
            if xm <= x[i] <= xM and ym <= y[j] <= yM:
                M[j, i] = T_init_max
    return M
...
for n in range(0, Nt):
    ...
    T0 = square_window_2d(x, y, xm, xM, ym, yM)

    if g<10: #script for Blowtorch
        xm+=1
        xM+=1
        ym+=0
        yM+=0
